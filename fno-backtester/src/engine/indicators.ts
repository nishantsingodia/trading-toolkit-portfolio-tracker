...
/** Supertrend — (H+L)/2 +/- multiplier*ATR with band clamping. */
export function supertrend(
  ...
  for (let i = start; i < len; i++) {
    ...
    if (i > start && !isNaN(lowerBand[i - 1])) {
      lb = candles[i - 1].close > lowerBand[i - 1] ? Math.max(lb, lowerBand[i - 1]) : lb;
    }
    if (i > start && !isNaN(upperBand[i - 1])) {
      ub = candles[i - 1].close < upperBand[i - 1] ? Math.min(ub, upperBand[i - 1]) : ub;
    }
...
export function adx(candles: Candle[], period: number): AdxResult {
  ...
  if (period <= 0 || len < 2 * period + 1) return { adx: adxResult, plusDi, minusDi };
  ...
  plusDi[firstDiIdx] = smoothTR !== 0 ? (smoothPlusDM / smoothTR) * 100 : 0;
  minusDi[firstDiIdx] = smoothTR !== 0 ? (smoothMinusDM / smoothTR) * 100 : 0;
  const diSum = plusDi[firstDiIdx] + minusDi[firstDiIdx];
  dx[firstDiIdx] = diSum !== 0 ? (Math.abs(plusDi[firstDiIdx] - minusDi[firstDiIdx]) / diSum) * 100 : 0;
  ...
  if (adxStart < len) {
    let adxSum = 0;
    for (let i = period; i <= adxStart; i++) adxSum += dx[i];
...
export function stochastic(candles: Candle[], kPeriod: number, dPeriod: number): StochasticResult {
  ...
  if (kValues.length >= dPeriod) {
    ...
    for (let i = 0; i < len; i++) {
      if (!isNaN(k[i])) {
        if (!isNaN(dSma[kIdx])) d[i] = dSma[kIdx];
        kIdx++;
      }
    }
  }
...
export function crossover(a: number[], b: number[], i: number): boolean {
  if (i < 1) return false;
  return !isNaN(a[i]) && !isNaN(b[i]) && !isNaN(a[i - 1]) && !isNaN(b[i - 1]) &&
    a[i - 1] <= b[i - 1] && a[i] > b[i];
}
...
/** Daily return percentage for each candle. */
